...
class CalculatorTests extends TestCase
{
    ...
    {
        $result = $this->calc->calculate('-2+3*(-3+4)');
    ...
    {
        $result = $this->calc->escapeSingleOperators('-2+3');
        $this->assertEquals('0-2+3', $result);
    }

    public function test_10()
    {
        $result = $this->calc->escapeSingleOperators('-2+3*(-3+4)');
        $this->assertEquals('0-2+3*(0-3+4)', $result);
    }
}